...
#### Topic correlation and community detection
...
Observation is that there appears to be 11 main clusters and 6 smaller clusters.  
...
This shows 24 different clusters / comunities. Plus multiple additional communities for each of the isolated topics (i.e., topics that do not have any connections).  
  
The algorithm found the 15 main clusters, but also added additional smaller clusters that don't seem to fit well in any of the 9 main clusters.  
...
```{r dynamic vis network}
data <- toVisNetworkData(graph)
nodes <- data[[1]]
edges <- data[[2]]

# delete nodes that dont have a connection (degree == 0)
...
The second dropdown highlights the communities detected in our algorithm. Play around with this menu. Using the topic names (zoom in with mouse scroll), you can interpret what the topic community seem to be.  
  
The 5 largest seems to be:
1. Related to bonus discount at the time of purchase (drak green in colour, cluster 2)  
2. Related to Visa (light blue, cluster 1)  
3. Related ro Work permit (light yellow, cluster 2)  
4. related to job level (Drak blue, cluster 5)  
  
There are other smaller communities/ clusters that are shown in the diagram.  
...
sentiment_syuzhet <- get_sentiment(comp_topicDF$Complaint.Text)
sentiment_afinn <- get_sentiment(comp_topicDF$Complaint.Text,method="afinn")
sentiment_bing <- get_sentiment(comp_topicDF$Complaint.Text,method="bing")
sentiment_nrc <- get_sentiment(comp_topicDF$Complaint.Text,method="nrc")
