...
for(top_id in unique(comp_topicDF$topic_id))
{
  ...
  temp_sum_df <- lexRank(temp_df4sum[,"Complaint.Text"],
                         threshold = 0.5,
                         n=ceiling(nrow(temp_df4sum) * 0.1),
                         Verbose = FALSE)
  
  topic_summary <- c(topic_id=top_id,
                     topic_summary_txt = paste(temp_sum_df[,"sentence"],
                                               collapse="  \n"))
...
for(i in 1:nrow(comp_topicDF))
{
  ...
  if(length(gregexpr('[[:alnum:] ]{5}[.!?]', 
                     comp_topicDF[i,"Complaint.Text"])[[1]]) > 2)
  {
    comp_sum <- c(comp_sum,paste(genericSummary(comp_topicDF[i,"Complaint.Text"],
                                                k=2,
                                                min=5),
                                               collapse="  \n"))
  } else {
...
comp_topicDF <- cbind.data.frame(comp_topicDF,complaint_summary = comp_sum,
                                 stringsAsFactors = FALSE)
