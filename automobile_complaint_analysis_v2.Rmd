                            ...
                            breakdown = FALSE,...) 
{
    ...
    summ.text <- paste(unique(summary.sentences),collapse=".  \n")
    com_pct <- round(((stri_count(text,regex="\\S+") - 
                         stri_count(summ.text,regex="\\S+")) /
                stri_count(text,regex="\\S+")) * 100,digits = 2)
    list(summary.text= summ.text,
                            ...
                            min_word_sentence = 3)
{
  ...
  sentences <- sentences[(stri_count(sentences,regex="\\S+") > min_word_sentence) &
                           (nchar(sentences) > 
                           (stri_count(sentences,regex="\\S+") * 2))]
  summ_list <- tryCatch({
                          list(summary.text = paste(
                                unique(
                                  lexRank(text,
                                   threshold = 0.2,
                                   n=min_sum_len,
                                   sentencesAsDocs = TRUE,
                                   Verbose = FALSE)$sentence),
                                collapse=".  \n"),
                               is_summarised = "Y"
                          )
                        },
                          error = function(e) {
                            return(
                            list(summary.text = paste(sentences,collapse=".  \n"),
                                 is_summarised = "N")
                              )
                            }
                      )
    ...
    com_pct <- round(((stri_count(text,regex="\\S+") - 
                         stri_count(summ_list$summary.text,regex="\\S+")) /
                stri_count(text,regex="\\S+")) * 100,digits = 2)
...
comp_sum_df <- data.frame()

for(i in 1:nrow(compDF))
{
  #print(i)
  
   comp_sum <- genericSummary_LSA(compDF[i,"Complaint.Text"],
                                                k_min=2,
                                                min=5)
  
  complaint_summary_LSA <- comp_sum$summary.text
  LSA_summarised <- comp_sum$is_summarised
  LSA_compression_pct <- comp_sum$compression_pct
  comp_sum_df <-  rbind.data.frame(comp_sum_df,c(complaint_summary_LSA,
                                                 LSA_summarised,
                                                 LSA_compression_pct),
                                   stringsAsFactors = FALSE)
...
names(comp_sum_df) <- c("complaint_summary_LSA","LSA_summarised","LSA_compression_pct")
compDF <- cbind.data.frame(compDF,comp_sum_df,
                                 stringsAsFactors = FALSE)
...
for(i in 1:nrow(compDF))
{
  ...
  comp_sum <- lexrank_summary(compDF[i,"Complaint.Text"],
                            min_sum_len = 2,
                            min_word_sentence = 5)
  
  complaint_summary_lexrank <- comp_sum$summary.text
  lexrank_summarised <- comp_sum$is_summarised
  lexrank_compression_pct <- comp_sum$compression_pct
...
compDF <- cbind.data.frame(compDF,comp_sum_lex_df,
                                 stringsAsFactors = FALSE)
