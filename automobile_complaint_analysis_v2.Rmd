...
genericSummary_LSA <- function (text, k_min,
                                k_max_pct = 0.1,
                                split = "\\. |! |\\? |\\\n{2,}", 
                                min = 5, 
                                breakdown = FALSE,...) 
{
  ...
  sentencesDF <- cbind.data.frame(snum = which((stri_count(sentences,
                                                           regex="\\S+") > min) &
                                  (nchar(sentences) > 
                    ...
                    sentences = sentences[(stri_count(sentences,regex="\\S+") > min) &
                                           (nchar(sentences) > 
                                              (stri_count(sentences,regex="\\S+") * 2))],
                                stringsAsFactors = F)
  if(nrow(sentencesDF) > k)
  {
    td = tempfile()
    dir.create(td)
    for (i in 1:nrow(sentencesDF)) {
      docname <- paste("sentence", sentencesDF[i,"snum"], ".txt", sep = "")
      write(sentencesDF[i,"sentences"], file = paste(td, docname, sep = "/"))
    }
    A <- textmatrix(td, ...)
    rownames <- rownames(A)
    colnames <- colnames(A)
    ...
    colnames(A) <- colnames
    unlink(td, T, T)
    Vt <- lsa(A, dims = nrow(sentencesDF))$dk
    ...
    summary.sentencesDF <- sentencesDF[sentencesDF$snum %in% snum,]
    names(summary.sentencesDF) <- c("snum", "summary.sentences")
    is_summarised <- "Y"
  }else{
    summary.sentencesDF <- sentencesDF
    names(summary.sentencesDF) <- c("snum", "summary.sentences")
    is_summarised <- "N"
  }
  summ.textDF <- unique(summary.sentencesDF)
  com_pct <- round(((stri_count(text,regex="\\S+") - 
                       stri_count(paste(summ.textDF$summary.sentences,collapse = ".  \n"),
                                  regex="\\S+")) /
                      stri_count(text,regex="\\S+")) * 100,digits = 2)
  list(summary.text= summ.textDF,
       is_summarised = is_summarised,
       compression_pct = com_pct)
  
}


lexrank_summary <- function(text,
                            split = "\\. |! |\\? |\\\n{2,}",
                            min_sum_len,
                            max_sum_pct = 0.1,
                            min_word_sentence = 3)
{
  sentences <- unlist(strsplit(as.character(text), 
                               split = split, fixed = F))
  
  
  if (ceiling(length(sentences) *  max_sum_pct) > min_sum_len)
  {
    min_sum_len <- ceiling(length(sentences) *  max_sum_pct)
  }
  
  sentencesDF <- cbind.data.frame(snum = which((stri_count(sentences,
                                              regex="\\S+") > min_word_sentence) &
                                                 (nchar(sentences) > 
                                                    (stri_count(sentences,
                                                                regex="\\S+") * 2))),
                                  sentences = sentences[(stri_count(sentences,
                                                  regex="\\S+") > min_word_sentence) &
                                                        (nchar(sentences) > 
                                                             (stri_count(sentences,
                                                              regex="\\S+") * 2))],
  ...
  summ_list <- tryCatch({
              temp.textDF <- lexRank(text,
                                     threshold = 0.2,
                                     n=min_sum_len,
                                     sentencesAsDocs = TRUE,
                                     Verbose = FALSE)
              summ.textDF <- cbind.data.frame(snum = as.numeric(str_split_fixed(
                                          temp.textDF$sentenceId,"_",2)[,2]),
                                              summary.sentences = temp.textDF$sentence,
                                              stringsAsFactors = F)
              
              list(summary.text = unique(summ.textDF),
                is_summarised = "Y"
              )
            },
            error = function(e) {
              ...
              names(summary.sentencesDF) <- c("snum", "summary.sentences")
              return(
                list(summary.text = summary.sentencesDF,
                     is_summarised = "N")
              )
            }
            )
  ...
  com_pct <- round(((stri_count(text,regex="\\S+") - 
                       stri_count(paste(summ_list$summary.text$summary.sentences,
                                        collapse = ".  \n"),
                                  regex="\\S+")) /
                      stri_count(text,regex="\\S+")) * 100,digits = 2)
  
  summ_list$compression_pct <- com_pct
...
## Are sentiments scores different from each other?
...
## Sentiments - a different way


```{r sentiment sentence}
