...
for(i in 1:nrow(compDF))
{
  ...
  sentences <- unlist(strsplit(compDF[i,"Complaint.Text"], 
                               split = "\\.|!|\\?|\\\n{2,}", fixed = F))
  min_sum_len <- 2
  max_sum_pct <- 0.1
  ...
  sentences <- sentences[(stri_count(sentences,regex="\\S+") > min_word_sentence) &
                           (nchar(sentences) > 
                           (stri_count(sentences,regex="\\S+") * 2))]
  comp_sum <- tryCatch(paste(
                            unique(
                              lexRank(compDF[i,"Complaint.Text"],
                               threshold = 0.2,
                               n=min_sum_len,
  ...
  comp_sum_lex <- c(comp_sum_lex,comp_sum)
}

compDF <- cbind.data.frame(compDF,complaint_summary_lexrank = comp_sum_lex,
                                 stringsAsFactors = FALSE)
